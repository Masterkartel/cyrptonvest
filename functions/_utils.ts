...
export type Env = {
  DB: D1Database;
  AUTH_COOKIE_SECRET: string; // kept for backwards compat
  ADMIN_EMAIL?: string;
...
export function buildCookieFromSid(
  ...
  try {
    ...
    // Avoid Domain on *.pages.dev and localhost
...
/* Build an expiring cookie (for logout) */
...
/** Salted sha256 (fallback when bcrypt is unavailable) */
...
/** Preferred hasher for resets: bcrypt if available; otherwise s256 */
...
export async function getUserFromSession(req: Request, env: Env): Promise<Session | null> {
  try {
    ...
    const role: "user" | "admin" = adminEmail && email === adminEmail ? "admin" : "user";
...
function equalsCI(a: string | undefined, b: string | undefined) {
  return (a || "").toLowerCase() === (b || "").toLowerCase();
}

/** Require admin (role OR email matches ADMIN_EMAIL) */
export async function requireAdmin(req: Request, env: Env) {
  const sess = await getUserFromSession(req, env);
  const adminEmail = (env.ADMIN_EMAIL || "").toLowerCase();
  const isAdmin = !!sess && (sess.role === "admin" || equalsCI(sess.email, adminEmail));
  if (!isAdmin) throw json({ error: "Forbidden" }, 403);
  return sess!;
}

/* ═════════ Back-compat shims (sessions) ═════════ */
