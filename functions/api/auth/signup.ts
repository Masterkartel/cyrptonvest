...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  try {
    ...
    // 6) Fire-and-forget: send welcome email with homepage tone
    try {
      const base =
      ...
      linear-gradient(180deg,#101934,#0c1226);border:1px solid #27335a;border-radius:16px;overflow:hidden">
        ...
        <tr><td style="padding:22px">
          ...
          <div style="border:1px solid #1d2640;background:#0f1629;border-radius:12px;padding:12px;margin-top:12px">
            <p style="margin:0 0 6px;color:#9aa4b2"><strong style="color:#e6edf3">Next steps</strong></p>
      ...
      // Non-blocking
      sendEmail(env, email, "Welcome to Cyrptonvest 🎉", html)
        .catch((e: any) => console.error("[welcome email]", e));
    } catch (e) {
      console.error("[welcome email] build/send error", e);
    }
