...
import {
  ...
  randomTokenHex,
  sendEmail,
  createSession,          // ⬅️ use this instead of setCookie
  type Env
} from "../../_utils";
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  try {
    ...
    // 4) Hash password and insert (timestamps as INTEGER epoch seconds)
    ...
    await env.DB.prepare(
      `INSERT INTO users (id, email, password_hash, created_at)
       VALUES (?, ?, ?, ?)`
    ).bind(id, email, hash, created_at).run();

    // 5) Start a session (returns full Set-Cookie string)
    const role = email === (env.ADMIN_EMAIL || "").toLowerCase() ? "admin" : "user";
    const setCookie = await createSession(
      env,
      { sub: id, email, role, iat: Math.floor(Date.now() / 1000) as any },
      request,
      60 * 60 * 24 * 14 // 14 days
    );

    // 6) Fire-and-forget welcome email (doesn't block signup)
    (async () => {
      try {
    ...
    // 7) Respond with Set-Cookie header
    return new Response(JSON.stringify({ ok: true, user: { id, email, role } }), {
      status: 200,
      headers: { "content-type": "application/json", "set-cookie": setCookie },
    });
  } catch (err: any) {
    const msg = String(err?.message || "");
    if (/(UNIQUE|constraint)/i.test(msg)) return bad("Email is already registered", 409);
