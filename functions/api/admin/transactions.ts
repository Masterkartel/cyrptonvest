...
function normalizeStatus(raw: string): "pending" | "completed" | "failed" | null {
  const s = String(raw || "").trim().toLowerCase();
  if (s === "approve" || s === "approved" || s === "completed") return "completed";
  if (s === "reject"  || s === "rejected"  || s === "failed")    return "failed";
 ...
 *  3) Try to flip status with guard TRIM(LOWER(status))='pending'.
 *     If no row updated, retry without the guard (handles stray whitespace/casing).
 *  4) If new status is 'completed', adjust the exact wallet (by wallet_id).
 *     On any wallet error, revert tx status back to 'pending'.
 */
export const onRequestPatch: PagesFunction<Env> = async (ctx) => {
  try {
    ...
    if (!id || !newStatus) return bad("Provide id and a valid status", 400);

    // 1) Load tx (need wallet_id, amount, kind, current status)
    ...
    // 2) Flip status FIRST.
    //    Try with a strict pending guard, then retry without it (handles trailing spaces or odd casing).
    let upd = await ctx.env.DB.prepare(
      `UPDATE txs
          SET status = ?
        WHERE id = ?
    ...
    // 3) If completing, adjust wallet AFTER successful status flip
    if (newStatus === "completed") {
      ...
      if (delta < 0) {
        ...
        if (current < Math.abs(delta)) {
          // revert status if insufficient funds
      ...
      if (updWallet.meta.changes !== 1) {
        // revert status if wallet update failed
        await ctx.env.DB.prepare(`UPDATE txs SET status='pending' WHERE id = ?`).bind(id).run();
